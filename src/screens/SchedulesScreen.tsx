...
import { AlarmSoundId, DoseEvent, Medicine, SnoozeMinutes } from '../types/medication';
...
{todayDoses.length === 0 ? (
         ...
         todayDoses.map(dose => {
           ...
           const dk = `${m.id}#${dose.scheduledTime}`;
           const soundLabel =
             ALARM_SOUND_OPTIONS.find(sound => sound.id === m.alarmSound)
               ?.label || 'Normal';

           return (
             <View key={dk} style={appStyles.alarmCard}>
                 ...
                 <View style={appStyles.scheduleInfo}>
                   <View style={appStyles.rowBetween}>
                     <View style={appStyles.medicineHeaderInfo}>
                       <Text style={appStyles.medicineName}>{m.name}</Text>
                       <Text style={appStyles.softText}>
                         {m.medicineType || 'Medicamento'} -{' '}
                         {FREQUENCIES.find(f => f.hours === m.frequency)?.label}
                       </Text>
                       {m.treatmentDays ? (
                         (() => {
                           const start = new Date(m.createdAt);
                           const end = new Date(start);
                           end.setDate(end.getDate() + m.treatmentDays!);
                           const now = new Date();
                           ...
                           return (
                             <Text style={appStyles.softText}>
                               {remaining > 0
                                 ? `${m.treatmentDays} dias (restan ${remaining})`
                     ...
                     <Switch
                       value={m.alarmEnabled}
                       onValueChange={value =>
                         onUpdateMedicineAlarm(m.id, { alarmEnabled: value })
                       }
                     />
                   </View>
                   <View style={appStyles.inlineIconText}>
                     ...
                     <Text style={appStyles.softText}>
                       {m.alarmEnabled ? 'Alarma activa' : 'Alarma apagada'}
                     </Text>
                   </View>
                   <View style={appStyles.inlineIconText}>
                     <AppIcon
                     ...
                     />
                     <Text style={appStyles.softText}>
                       {soundLabel} - posponer {m.snoozeMinutes || 10} min
                     </Text>
                   </View>
                 </View>
               </View>
                 ...
                 {ALARM_SOUND_OPTIONS.map(sound => (
                   <TouchableOpacity
                     ...
                     style={[
                       ...
                       m.alarmSound === sound.id
                         ? appStyles.settingsChipActive
                         : null,
                     ]}
                     onPress={() =>
                       onUpdateMedicineAlarm(m.id, { alarmSound: sound.id })
                     }
                   >
                     <Text
                       style={[
               ...
               <View style={appStyles.settingsChipRow}>
                 {SNOOZE_OPTIONS.map(minutes => (
                   <TouchableOpacity
                     ...
                     style={[
                       appStyles.settingsChip,
                       m.snoozeMinutes === minutes
                         ? appStyles.settingsChipActive
                         : null,
                     ]}
                     onPress={() =>
                       onUpdateMedicineAlarm(m.id, { snoozeMinutes: minutes })
                     }
                   >
                     <Text
                       style={[
